...
export default function TranscribePage() {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={<><Mic size={24} className="text-indigo-400 inline" /> 語音轉文字</>}
        description="粵語優化 · faster-whisper · pyannote 說話者分離 · AI 摘要分析"
      />

      <Tabs
        tabs={[
          { id: "transcribe", label: "轉錄", icon: Mic },
          { id: "files", label: "檔案", icon: FolderClosed },
          { id: "analyze", label: "AI 分析", icon: Brain },
        ]}
        activeTab={tab}
        onChange={(t) => setTab(t as "transcribe" | "files" | "analyze")}
        variant="pills"
      />
          ...
          <div className="space-y-4">
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><FileAudio size={14} />選擇音檔</CardTitle></CardHeader>
              <div className="px-4 pb-4 space-y-3">
                <div className={cn("border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-colors", dragOver ? "border-indigo-400 bg-indigo-500/10" : "border-[var(--color-border)] hover:border-indigo-500/50")}
                  onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
                  onDrop={e => { e.preventDefault(); setDragOver(false); Array.from(e.dataTransfer.files).forEach(handleUpload); }}
                  onClick={() => fileInputRef.current?.click()}>
                  {uploading ? <Loader2 size={20} className="mx-auto animate-spin text-indigo-400" /> : <Upload size={20} className="mx-auto text-[var(--color-text-muted)]" />}
                  <p className="text-xs text-[var(--color-text-muted)] mt-1">拖放或點擊上傳 (MP3/M4A/WAV/MP4)</p>
                </div>
                ...
                <div className="max-h-[180px] overflow-y-auto space-y-1">
                  {audioFiles.map(f => (
                    <div key={f.path} className={cn("flex items-center gap-2 px-3 py-2 rounded-lg text-xs cursor-pointer", selectedAudio === f.path ? "bg-indigo-500/15 text-indigo-400" : "hover:bg-[var(--color-surface-elevated)] text-[var(--color-text-secondary)]")}
                      onClick={() => setSelectedAudio(f.path)}>
                      <FileAudio size={12} /><span className="truncate flex-1">{f.name}</span><span className="text-[var(--color-text-muted)]">{fmtSize(f.size)}</span>
                    </div>))}
                </div>
              </div>
            </Card>
            ...
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Cpu size={14} />設定</CardTitle></CardHeader>
              <div className="px-4 pb-4 space-y-3">
                <div><label className="text-[10px] text-[var(--color-text-muted)]">模型</label>
                  <select value={model} onChange={e => setModel(e.target.value)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1">
                    {MODELS.map(m => <option key={m.key} value={m.key}>{m.label}</option>)}</select>
                </div>
                <div><label className="text-[10px] text-[var(--color-text-muted)]">語言</label>
                  <select value={language} onChange={e => setLanguage(e.target.value)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1">
                    {LANGUAGES.map(l => <option key={l.key} value={l.key}>{l.label}</option>)}</select>
                </div>
                <div className="flex items-center justify-between">
                  <div><span className="text-xs flex items-center gap-1"><Users size={12} />說話者分離</span><p className="text-[10px] text-[var(--color-text-muted)]">需 HF_TOKEN</p></div>
                  <button onClick={() => setDiarize(!diarize)} className={cn("w-10 h-5 rounded-full transition-colors relative", diarize ? "bg-indigo-500" : "bg-[var(--color-surface-overlay)]")}>
                    <span className={cn("w-4 h-4 rounded-full bg-white absolute top-0.5 transition-transform", diarize ? "left-5" : "left-0.5")} /></button>
                </div>
                {diarize && <div><label className="text-[10px] text-[var(--color-text-muted)]">人數 (0=自動)</label><input type="number" min={0} max={20} value={numSpeakers} onChange={e => setNumSpeakers(parseInt(e.target.value) || 0)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1" /></div>}
            ...
            {taskStatus && taskStatus.status !== "completed" && (
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><RefreshCw size={14} className="animate-spin" />進度</CardTitle></CardHeader>
                <div className="px-4 pb-4 space-y-3">
                  ...
                  <div className="w-full bg-[var(--color-surface-overlay)] rounded-full h-2"><div className="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all" style={{ width: `${taskStatus.progress || 5}%` }} /></div>
                  {taskStatus.step && <p className="text-xs text-[var(--color-text-muted)]">{taskStatus.step}</p>}
                </div>
              </Card>
            )}
            {result ? (
                ...
                <div className="px-4 pb-4 space-y-3">
                  <div className="grid grid-cols-4 gap-2">
                    {[{ l: "時長", v: result.duration_fmt }, { l: "段落", v: result.total_segments }, { l: "模型", v: result.model }, { l: "說話者", v: result.diarize && result.speakers?.length ? result.speakers.length : "—" }].map(s => (
                      <div key={s.l} className="bg-[var(--color-surface-elevated)] rounded-lg p-2 text-center"><p className="text-[10px] text-[var(--color-text-muted)]">{s.l}</p><p className="text-sm font-medium">{s.v}</p></div>))}
                  </div>
                  ...
                  <div className="max-h-[400px] overflow-y-auto bg-[var(--color-surface)] rounded-lg p-4 font-mono text-xs leading-relaxed">
                    {result.segments?.slice(0, 100).map((seg: Segment, i: number) => (
                      <p key={i} className="mb-1"><span className="text-[var(--color-text-muted)] mr-2">[{new Date(seg.start * 1000).toISOString().slice(11, 19)}]</span>{seg.speaker && <span className="text-indigo-400 font-semibold mr-1">{seg.speaker}:</span>}<span>{seg.text}</span></p>))}
                    {result.segments?.length > 100 && <p className="text-[var(--color-text-muted)] italic mt-2 text-center">... 僅顯示前 100 段</p>}
                  </div>
                </div>
              </Card>
            ) : (!taskStatus && !running && <Card className="h-full flex items-center justify-center"><div className="text-center py-12"><FileText size={48} className="mx-auto text-[var(--color-text-muted)] mb-3" /><p className="text-sm text-[var(--color-text-muted)]">選擇音檔後點擊開始轉錄</p></div></Card>)}
          </div>
          ...
          {tasks.length > 0 && <div className="xl:col-span-3"><Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Clock size={14} />最近任務</CardTitle></CardHeader>
            <div className="px-4 pb-4"><div className="space-y-1 max-h-[200px] overflow-y-auto">
              {tasks.slice(0, 10).map(t => (
                <div key={t.task_id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[var(--color-surface-elevated)] cursor-pointer" onClick={() => t.status === "completed" && loadResult(t.task_id)}>
                  {statusIcon(t.status)}<div className="flex-1 min-w-0"><p className="text-xs truncate">{t.filename}</p><p className="text-[10px] text-[var(--color-text-muted)]">{t.model_key} · {fmtDate(t.created)}</p></div>
          ...
          <div className="space-y-4">
            ...
            <Card><div className="p-4 max-h-[500px] overflow-y-auto space-y-1">
              {(fileCat === "transcripts" ? transcripts : summaries).map(f => (
                <div key={f.path} className={cn("flex items-center gap-2 px-3 py-2 rounded-lg text-xs cursor-pointer hover:bg-[var(--color-surface-elevated)]", fileView?.name === f.name ? "bg-indigo-500/10 text-indigo-400" : "text-[var(--color-text-secondary)]")}
                  onClick={() => viewFile(f.path)}>
                  {f.ext === ".json" ? <FileText size={12} /> : <ScrollText size={12} />}
                  <span className="truncate flex-1">{f.name}</span>
                  <span className="text-[var(--color-text-muted)]">{fmtSize(f.size)}</span>
              ...
              {((fileCat === "transcripts" ? transcripts : summaries).length === 0) && <p className="text-xs text-[var(--color-text-muted)] text-center py-8">尚無檔案</p>}
            </div></Card>
          </div>
            ...
            {fileView ? (
              <div>
                <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)]">
                  ...
                  <button onClick={() => setFileView(null)} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]"><XCircle size={16} /></button>
                </div>
                <div className="p-4 max-h-[600px] overflow-y-auto font-mono text-xs leading-relaxed whitespace-pre-wrap">{fileView.content}</div>
              </div>
            ) : (
              <div className="flex items-center justify-center h-[400px]"><div className="text-center"><Eye size={48} className="mx-auto text-[var(--color-text-muted)] mb-3" /><p className="text-sm text-[var(--color-text-muted)]">點擊左側檔案即可預覽</p></div></div>
            )}
          </Card>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="space-y-4">
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Bot size={14} />AI 分析設定</CardTitle></CardHeader>
              <div className="px-4 pb-4 space-y-3">
                <div><label className="text-[10px] text-[var(--color-text-muted)]">選擇轉錄檔案</label>
                  <select value={analyzeFile} onChange={e => setAnalyzeFile(e.target.value)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1">
                    <option value="">— 選擇 TXT 檔案 —</option>
                    {transcripts.filter(f => f.ext === ".txt").map(f => <option key={f.path} value={f.path}>{f.name}</option>)}</select>
                </div>
                <div><label className="text-[10px] text-[var(--color-text-muted)]">LLM 模型</label>
                  <select value={llmProvider} onChange={e => setLlmProvider(e.target.value)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1">
                    {LLM_PROVIDERS.map(p => <option key={p.key} value={p.key}>{p.label} — {p.desc}</option>)}</select>
                </div>
                <div><label className="text-[10px] text-[var(--color-text-muted)]">錄音類型</label>
                  <select value={recordingType} onChange={e => setRecordingType(e.target.value)} className="w-full bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-xs mt-1">
            ...
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><FolderOpen size={14} />快速預覽</CardTitle></CardHeader>
              <div className="px-4 pb-4">
                <p className="text-xs text-[var(--color-text-muted)]">先到「檔案」分頁查看轉錄內容，確認後再回到此處進行 AI 分析。</p>
              </div>
            </Card>
          </div>
          <div className="xl:col-span-2">
            {analyzeResult ? (
              <Card><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-sm flex items-center gap-2"><Brain size={14} className="text-green-400" />AI 分析結果</CardTitle><a href={`/api/transcribe?sub=download&path=${encodeURIComponent(analyzeResult.txt_path)}`} className="text-xs text-indigo-400 flex items-center gap-1"><Download size={12} />下載摘要</a></div></CardHeader>
                <div className="px-4 pb-4">
                  {analyzeResult.summary?.overall_summary && <div className="mb-4"><p className="text-xs font-medium text-indigo-400 mb-1">整體摘要</p><p className="text-xs text-[var(--color-text-primary)] leading-relaxed">{analyzeResult.summary.overall_summary}</p></div>}
                  {analyzeResult.summary?.key_topics?.length > 0 && <div className="mb-4"><p className="text-xs font-medium text-indigo-400 mb-2">主要議題</p>
                    {analyzeResult.summary.key_topics.map((t: any, i: number) => (
                      <div key={i} className="bg-[var(--color-surface-elevated)] rounded-lg p-3 mb-2"><p className="text-xs font-medium mb-1">{t.topic}</p><p className="text-xs text-[var(--color-text-secondary)]">{t.discussion}</p>{t.decisions?.length > 0 && <div className="mt-2"><p className="text-[10px] text-[var(--color-text-muted)] mb-1">決議：</p>{t.decisions.map((d: string, j: number) => <Badge key={j} variant="success" className="text-[10px] mr-1 mb-1">{d}</Badge>)}</div>}</div>))}
                  </div>}
                  {analyzeResult.summary?.action_items?.length > 0 && <div className="mb-4"><p className="text-xs font-medium text-indigo-400 mb-2">待辦事項</p>
                    {analyzeResult.summary.action_items.map((a: any, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs py-1"><CheckCircle2 size={12} className="text-[var(--color-text-muted)]" /><span>{a.item}</span>{a.assignee && <Badge variant="info" className="text-[10px]">{a.assignee}</Badge>}{a.deadline && <span className="text-[10px] text-amber-400">({a.deadline})</span>}</div>))}
                  </div>}
                  <p className="text-[10px] text-[var(--color-text-muted)]">分析引擎：{analyzeResult.provider} / {analyzeResult.model} · 儲存於 summaries/ 資料夾</p>
                </div>
              </Card>
            ) : (
              <Card className="h-full flex items-center justify-center"><div className="text-center py-12"><Brain size={48} className="mx-auto text-[var(--color-text-muted)] mb-3" /><p className="text-sm text-[var(--color-text-muted)]">選擇轉錄 TXT 檔案 → 選 LLM → 開始分析</p><p className="text-xs text-[var(--color-text-muted)] mt-1">AI 會自動生成結構化摘要、議題分類、待辦事項</p></div></Card>
            )}
          </div>
        </div>
      )}
