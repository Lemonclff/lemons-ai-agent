...
import { TrendingDown, TrendingUp, Minus, Trash2, Plus, Scale, Calendar, Pencil, Target, Flame, Activity, Apple, Dumbbell } from "lucide-react";
...
const ACTIVITY_OPTIONS = [
  { value: "sedentary",   label: "Sedentary",  sub: "little or no exercise",  mult: 1.20, icon: "🪑" },
  { value: "light",       label: "Light",      sub: "1−3 days/week",           mult: 1.375, icon: "🚶" },
  { value: "moderate",    label: "Moderate",   sub: "3−5 days/week",           mult: 1.55, icon: "🏃" },
  { value: "active",      label: "Active",     sub: "6−7 days/week",           mult: 1.725, icon: "💪" },
  { value: "very_active", label: "Athlete",    sub: "intense training, 2×/day", mult: 1.90, icon: "🏋️" },
];

const GOAL_OPTIONS = [
  { value: "lose_fast", label: "⬇️ Aggressive Cut", sub: "~20% deficit", color: "text-rose-400" },
  { value: "lose",      label: "⬇️ Moderate Cut",  sub: "~15% deficit", color: "text-orange-400" },
  { value: "maintain",  label: "⚖️ Maintain",      sub: "weight stable", color: "text-sky-400" },
  { value: "gain",      label: "⬆️ Lean Bulk",     sub: "+10% surplus", color: "text-green-400" },
  { value: "gain_fast", label: "⬆️ Aggressive Bulk", sub: "+15% surplus", color: "text-emerald-400" },
];
...
export function ProfileTab({
  ...
  const MacroBadge = ({ label, value, unit, color }: { label: string; value: number; unit: string; color: string }) => (
    <div className="flex flex-col items-center p-2 rounded-lg bg-[var(--color-surface-elevated)]/30">
    ...
    <div className="space-y-4 max-w-[560px]">
      ...
      <div className="border border-[var(--color-border)] rounded-lg p-4">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-7 h-7 rounded-lg bg-[var(--color-accent)]/15 flex items-center justify-center"><Flame size={14} className="text-[var(--color-accent)]" /></div>
          <h3 className="text-[13px] font-semibold text-[var(--color-text-secondary)]">TDEE Calculator</h3>
          {previewTdee && <span className="ml-auto text-[13px] font-bold text-[var(--color-accent)] tabular-nums">{previewTdee} kcal</span>}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 max-md:grid-cols-1 mb-3">
          {[
            ...
            <div key={f.key}>
              <label className="text-[10px] text-[var(--color-text-muted)] block mb-0.5">{f.label}</label>
              {f.type === "select" ? (
          ...
          <div>
            <label className="text-[10px] text-[var(--color-text-muted)] block mb-0.5">Body Fat % <span className="opacity-40">(opt)</span></label>
          ...
          <div>
            ...
            <select value={profile.activity_level} onChange={e => setProfile(p => ({ ...p, activity_level: e.target.value }))}
              className="w-full px-2.5 py-1.5 text-[12px] bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg outline-none">
              {ACTIVITY_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.icon} {o.label} — {o.sub}</option>)}
            </select>
          </div>
          <div>
            <label className="text-[10px] text-[var(--color-text-muted)] block mb-0.5">Goal</label>
        ...
        {bmr && (
          <div className="flex items-center gap-3 mb-3 px-3 py-2 rounded-xl bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/10 text-[12px]">
        ...
        <button onClick={saveProfile} className="w-full py-2 text-[13px] font-semibold rounded-xl bg-[var(--color-accent)] text-white hover:opacity-90 transition-opacity">
      ...
      <div className="border border-[var(--color-border)] rounded-lg overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 bg-[var(--color-surface-elevated)]/30 border-b border-[var(--color-border)]/50">
          <div className="w-7 h-7 rounded-lg bg-indigo-500/15 flex items-center justify-center"><Scale size={14} className="text-indigo-400" /></div>
          <span className="text-[13px] font-semibold text-[var(--color-text-secondary)]">Weight Tracking</span>
          {weightStats && <span className="ml-auto text-[11px] text-[var(--color-text-muted)] tabular-nums">{weightStats.count} entries</span>}
        </div>

        {weightLog.length >= 2 && (
          <div className="px-4 pt-3">
            <div className="flex items-baseline gap-3 mb-1">
              <span className="text-[28px] font-bold text-[var(--color-text-primary)] tabular-nums leading-none">{weightStats?.latest}</span>
          ...
          <div className="flex gap-2">
            ...
            <div className="flex-1">
              ...
              <input type="date" value={weightDate} onChange={e => setWeightDate(e.target.value)}
                className="w-full px-2 py-2.5 text-[12px] bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl outline-none" />
            </div>
          </div>
          <div className="flex gap-2 mt-2">
            <div className="flex-[3]">
              <input type="text" value={weightNote} onChange={e => setWeightNote(e.target.value)}
                ...
                className="w-full px-3 py-2 text-[12px] bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl outline-none" />
            </div>
            <button onClick={addWeight} disabled={addingWeight || !newWeight}
              className="flex-1 py-2 text-[13px] font-semibold rounded-xl bg-indigo-500 text-white hover:bg-indigo-400 disabled:opacity-30 transition-all flex items-center justify-center gap-1.5">
              <Plus size={15} /> Log
            </button>
          </div>
        </div>
        ...
        {weightLog.length > 1 && (
          <div className="border-t border-[var(--color-border)]/30 max-h-[220px] overflow-y-auto">
            {weightLog.map((e, i) => {
              ...
              return (
                <div key={e.id} className="flex items-center gap-2 px-4 py-2 text-[13px] hover:bg-[var(--color-surface-elevated)]/20 group transition-colors border-b border-[var(--color-border)]/10 last:border-0">
                  ...
                  <button onClick={() => deleteWeight(e.id)}
                    className="opacity-0 group-hover:opacity-100 p-1 rounded text-[var(--color-text-muted)] hover:text-red-400 hover:bg-red-500/10 transition-all shrink-0">
                    <Trash2 size={13} />
                  </button>
                </div>
              );
