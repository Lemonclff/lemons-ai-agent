...
export function CaloriesOutTab({
  ...
  return (
    <div className="nutri-stagger space-y-4 max-w-[640px]">
          ...
          <div className="flex items-end gap-2 max-md:flex-wrap">
            <div className="flex-1 min-w-[120px]">
              ...
              <input value={exName} onChange={e => setExName(e.target.value)}
                ...
                className="w-full px-4 py-3 text-[16px] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl outline-none focus:border-green-500/50 transition-colors nutri-input-glow"
            ...
            <button onClick={handleAdd} disabled={!exName.trim()}
              className="min-h-[48px] px-6 text-[14px] font-semibold rounded-xl bg-green-500 text-white hover:bg-green-400 disabled:opacity-30 transition-all active:scale-95 shrink-0 max-md:w-full nutri-press">
              Add
            </button>
          </div>
        </div>
            ...
            {exercises.map((ex: any) => (
              <div key={ex.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[var(--color-surface-elevated)]/20 transition-colors group">
                ...
                <button onClick={() => deleteExercise(ex.id)}
                  className="opacity-0 group-hover:opacity-100 min-w-[40px] min-h-[40px] flex items-center justify-center rounded-xl hover:bg-red-500/10 text-[var(--color-text-muted)] hover:text-red-400 transition-all nutri-press">
                  <X size={15} />
                </button>
              </div>
            ))}
      ...
      {exercises.length === 0 && (
        <div className="text-center py-12 text-[13px] text-[var(--color-text-muted)] rounded-2xl border border-dashed border-[var(--color-border)]/50 nutri-card-hover">
