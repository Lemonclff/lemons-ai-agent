...
export async function GET(req: NextRequest) {
  ...
  const custom = await query(
    `SELECT id, food_name, food_name as display_name, calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g, 0 as fiber_per_100g, 'custom' as source, id::text as source_id, created_at FROM user_custom_foods WHERE user_id = $1 AND food_name ILIKE $2 LIMIT 10`,
    [uid, `%${q}%`]
  );
